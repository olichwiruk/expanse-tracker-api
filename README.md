...
1. A background job sends the image to a Vision LLM (Gemini/OpenAI/Claude).
1. The system validates the LLM's JSON structured output against a strict dry-struct defined contract.
1. If validation fails, the system automatically triggers a [Reflexion-based](https://arxiv.org/abs/2303.11366) rescue loop, feeding the error report back to the LLM and forcing an explicit reasoning step before its next attempt.
1. Successfully parsed and validated data is stored in PostgreSQL.
